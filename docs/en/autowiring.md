...
*English. Russian mirror: [`../ru/autowiring.md`](../ru/autowiring.md).*
...
2. It orders them into a deterministic init DAG with `InitOrdering` — the same Kahn sort, stable
   declaration-index tie-break, and `runBefore`/`runAfter` model as the hook DAG ([`injector.md`](injector.md)).
...
calls it ([`game-integration.md`](game-integration.md)) — the init methods run as plain, statically-linked
