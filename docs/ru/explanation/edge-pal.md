...
- **`InventoryAccess.selectedSlot()` / `heldItemId()`** — выбранный слот хотбара игрока (`-1` вне платформы),
  чтобы «ограничить предмет, который я держу» было одним гарантированно верным нажатием, а не выбором из хотбара.
- **`Platform.installForTesting(bridge)`** — опциональный обратимый тест-хук: headless-тест может предъявить
  локального игрока (`players().local()`), чтобы протестировать ветку «игрок есть», без записи в
  `META-INF/services`, которая изменила бы мост для всех тестов. Восстановление — `installForTesting(null)`.
  Только для тестов; продакшн всегда использует мост из `ServiceLoader`.
- **Направленный фасад `Network`** (серверный приём + отправка, с отправителем) живёт здесь, в
  `aetherium-edge`, потому что серверный обработчик несёт `PlayerHandle` — см. [network](network.md).
