...
### Персистентность MachineState ()

`MachineState` **долговечен и переживает перезапуск сервера.** `AetheriumMachineBlockEntity` обеспечивает его
через NBT блок-сущности: `saveAdditional` пишет `aeth_age`, UUID установившего и long/string состояния, а
`loadAdditional` восстанавливает их при перезагрузке чанка — поэтому накопленное состояние поведения (`ticks`
выше, счётчики эссенции, владелец) остаётся ровно там, где было, после `/reload` или полного перезапуска. Мод
не пишет NBT-код; он меняет `MachineState` (чистую карту без типов Minecraft), а загрузчик сохраняет это.
Состояние, которое машина *не* должна хранить между перезапусками, просто не следует класть в `MachineState`.